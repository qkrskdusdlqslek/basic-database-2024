...
-- 가격이 10,000 이상 20,000원 이하인 도서를 검색하시오
SELECT *
  FROM Book
...
SELECT bookname, publisher
  FROM Book
 WHERE bookname LIKE '축구%'  -- 축구라는 글자로 시작하는 도서 찾기(%)

SELECT bookname, publisher
  FROM Book
 WHERE bookname LIKE '%축구'  -- 축구라는 글자로 끝나는 도서가 없기 때문에 안나옴..

SELECT bookname, publisher
  FROM Book
 WHERE bookname LIKE '%축구%'  -- 축구라는 글자가 들어간 모든 도서 찾기

-- 두글자에 구로 끝나는 단어로 시작되는 출판사를 검색하시오
SELECT bookname, publisher
  FROM Book
 WHERE bookname LIKE '_구%'  -- _(무슨 글자든 한글자가 들어간) 구라는 글자로 시작하는 모든 도서 찾기