...
/* 5. 현재의 날짜 타입을 날짜 함수를 통해서 확인
      ...
      사원번호, 고용일자.. 단, 입사일 빠른 순으로 정렬하시오(18개행) */
...
/* 샘플문제. 60번 IT부서에 사원 급여를 12.3% 인상하기로 함. 정수만 반올림하여
            보고서를 작성, 사번, 이름, 급여, 인상된 급여(Increased Salary)(5개행)*/
SELECT EMPLOYEE_ID
     , FIRST_NAME + ' ' + LAST_NAME AS 'name'
     , SALARY
     , CONVERT(INT, ROUND(SALARY * 1.123, 0)) AS 'Increaased Salary'
  FROM employees
...
/* 8. 이름, 급여, 수당 여부에 따라서 연봉을 포함하여 출력, Salary + Commission /
      Salary Only, 연봉이 높은 순(107개행) */


/* 9. 이름, 입사일, 입사일의 요일 출력, 일요일부터 토요일 순으로 (107개행) */
SELECT FIRST_NAME + ' ' + LAST_NAME AS 'name'
     , HIRE_DATE
...
/* 11. 사원들의 업무 별 전체 급여 평균이 10,000$ 보다 큰 경우르 조회,
       업무, 급여 평균을 출력, 단 사원(CLERK)이 포함된 경우는 제외,
       전체 급여 높은 내림차순으로 실행(7개행)  */
SELECT JOB_ID
...
-- 서브쿼리
/* 사원의 급여 정보중 최소 급여를 받는 사원의 이름, 업무, 급여, 입사일 출력(21개행) */
...
-- 사원들의 부서별 급여 기준으로 내림차순으로 정렬, 순위를 같이 표시하시오(107행)
SELECT EMPLOYEE_ID
     , LAST_NAME
     , SALARY
     , DEPARTMENT_ID
