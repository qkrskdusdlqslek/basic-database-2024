...
class qtApp(QMainWindow):
    ...
    def btnNewClicked(self): # 신규버튼 클릭
        ...
        # 선택한 데이터에서 신규를 누르면 self.txtBookid 사용여부는 변경 필요
    ...
    def btnSaveClicked(self): # 저장버튼 클릭
        ...
        if isValid == False: # 위 입려값중에 하나라도 빈값이 존재
        ...
        if mode == 'I': # INSERT 경우
            ...
            query = f'''SELECT COUNT(*)
                          FROM Book
                         WHERE bookid = {bookid}''' # 현재 입력하고자하는 번호가 있는 확인 쿼리
    ...
    def makeTable(self, data): # tblBooks 위젯을 데이터와 컬럼을 생성해주는 함수
        ...
        for item in data:
            ...
            idItem.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter) # 오른쪽 정렬
            ...
            priceItem.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter) # 오른쪽 정렬
